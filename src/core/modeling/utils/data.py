...
def filter_by_min_counts(
    df: pd.DataFrame, min_user_ratings: int = 0, min_movie_ratings: int = 0, verbose: bool = True
) -> pd.DataFrame:
    """
    ...
    df_filtered = df[(df["user_id"].isin(valid_users)) & (df["movie_id"].isin(valid_movies))].copy()
...
def preprocess_id_mapping(df: pd.DataFrame, verbose: bool = True) -> Tuple[pd.DataFrame, IDMapping]:
    """
    ...
    id_mapping = IDMapping(
        user_to_idx=user_to_idx, idx_to_user=idx_to_user, movie_to_idx=movie_to_idx, idx_to_movie=idx_to_movie
    )
...
def find_movie_id_by_title(
    movie_title: str, df: pd.DataFrame, exact_match: bool = False, limit: int = 10
...
def get_movie_id(movie_title: str, df: pd.DataFrame) -> Optional[str]:
    """
