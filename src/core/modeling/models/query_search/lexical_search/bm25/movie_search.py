...
from .models import BM25SearchResult, _coerce_optional_str
...
class MovieBM25:
    ...
    def fit(self, movies_df):
        ...
        for _, row in movies_df.iterrows():
            title_value = _coerce_optional_str(
                row.get("total_title") or row.get("title")
            )
            genres_value = _coerce_optional_str(row.get(genres_col))
            if not genres_value:
                genres_value = _coerce_optional_str(row.get("genres"))
            meta = {
                "title": title_value,
                "genres": genres_value,
                "overview": _coerce_optional_str(row.get("overview")),
        ...
        for movie_id, data in sorted_results:
            ...
            if data["score"] >= min_score:
                result = BM25SearchResult(
                    movie_id=movie_id,
                    score=data["score"],
                    title=_coerce_optional_str(data["metadata"].get("title")),
                    genres=_coerce_optional_str(data["metadata"].get("genres")),
