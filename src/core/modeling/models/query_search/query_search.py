...
from .lexical_search import BM25Config, BM25SearchResult, MovieBM25
...
class QuerySearchPipeline:
    ...
    def __init__(self, bm25_config: Optional[BM25Config] = None, yaml_path: Optional[str] = None):
        """
    ...
    def search(
        ...
        Args:
            ...
            genre_filter: 장르 필터 리스트
            language_filter: 언어 필터 리스트
    ...
    def search_to_response(
        ...
        from app.api.schemas import QuerySearchResponse, SearchResultMovie

        results = self.search(query, top_k, min_score, min_rating, min_vote_count, genre_filter, language_filter)
        ...
        return QuerySearchResponse(query=query, total_results=len(search_result_movies), results=search_result_movies)
    ...
    def search_to_dict(
        ...
        Returns:
            검색 결과 딕셔너리
        """
        response = self.search_to_response(
            query, top_k, min_score, min_rating, min_vote_count, genre_filter, language_filter
        )
...
def create_search_pipeline(
    movies_df, bm25_config: Optional[BM25Config] = None, yaml_path: Optional[str] = None
) -> QuerySearchPipeline:
    """
...
def search_movies(
    pipeline: QuerySearchPipeline, query: str, top_k: int = 20, min_score: float = 0.0, return_type: str = "pydantic"
):
    """
    영화 검색 편의 함수
