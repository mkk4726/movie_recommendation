...
class PersonExtractionResult:
    ...
    def to_dict(self) -> dict:
        """딕셔너리로 변환"""
        return {"persons": self.persons, "count": len(self.persons)}
...
class GLiNERPersonExtractor:
    ...
    def __init__(self, model_name: str = "taeminlee/gliner_ko"):
        ...
        if not GLINER_AVAILABLE:
            raise ImportError("gliner 패키지가 설치되지 않았습니다. 다음 명령으로 설치하세요: pip install gliner")
    ...
    def extract_persons(self, text: str, threshold: float = 0.3, verbose: bool = False) -> PersonExtractionResult:
        """
        텍스트에서 사람 이름 추출
        ...
        try:
            ...
            result = PersonExtractionResult(persons=persons, raw_entities=entities)
