...
def split_train_test(
    ...
    logger.info("\n✅ 검증:")
    logger.info(f"  - Train/Test 모두에 포함된 사용자: {len(users_in_both):,}명 / {len(all_users):,}명 ({len(users_in_both)/len(all_users)*100:.1f}%)")
    logger.info(f"  - Train/Test 모두에 포함된 영화: {len(movies_in_both):,}개 / {len(all_movies):,}개 ({len(movies_in_both)/len(all_movies)*100:.1f}%)")
