...
from modeling.models.recommender.recommender import MovieRecommender
...
def recommend_for_user(
    ...
    # 추천 영화 가져오기
    recommendations_df = recommender.recommend_for_user(
        user_id=user_id,
        df_movies=df_movies,
        n_recommendations=n
    )
    ...
    # 사용자가 높게 평가한 영화 가져오기
    try:
        top_watched = recommender.df_trainset.loc[
            recommender.df_trainset['user_id'] == user_id
        ].nlargest(n, 'rating')
        
        if not top_watched.empty:
            top_watched_df = df_movies.merge(
                top_watched[['movie_id', 'rating']],
                on='movie_id',
                how='inner'
...
def similar_movies(
    ...
    try:
        ...
        # 영화 정보와 병합
        similar_df = df_movies.merge(
            similar_scores_df[['movie_id', 'similarity_score']],
            on='movie_id',
            how='inner'
        )
        ...
        if 'language' in filters and filters['language']:
            ...
            if 'language' in similar_df.columns:
                language_mask = similar_df['language'].apply(
                    lambda x: any(l in str(x) for l in language_list) if pd.notna(x) else False
                )
...
# MovieRecommender를 직접 사용할 수 있도록 클래스도 제공
class RecommenderService:
    """
    MovieRecommender를 래핑하는 클래스 (기존 코드 호환성용)
    """
    ...
    def recommend_for_user(
        ...
        """클래스 메서드로 recommend_for_user 호출"""
        return recommend_for_user(user_id, df_movies, n)
    ...
    def similar_movies(
        ...
        """클래스 메서드로 similar_movies 호출"""
        return similar_movies(movie_id, df_movies, n_recommendations, filters)
