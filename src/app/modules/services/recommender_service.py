...
def similar_movies(
    ...
    try:
        fetch_n = (
            n_recommendations * 60 if filters else n_recommendations
        )  # filter가 적용될거 감안하고 더 많이 뽑기 (뽑은 결과 -> 필터 적용되기 때문에)
...
class RecommenderService:
    ...
    def recommend_for_user(
        self, user_id: str, df_movies: pd.DataFrame, n: int = 10
    ) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """클래스 메서드로 recommend_for_user 호출"""
        return recommend_for_user(user_id, df_movies, n)

    def similar_movies(
        self, movie_id: str, df_movies: pd.DataFrame, n_recommendations: int = 10, filters: Optional[dict] = None
    ) -> pd.DataFrame:
