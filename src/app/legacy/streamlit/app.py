...
from user_system.firebase_config import get_firebase_manager, setup_firebase_config

# 페이지 설정 (반드시 최상단에 위치)
st.set_page_config(page_title="볼거 없나?", page_icon="🎬", layout="wide", initial_sidebar_state="expanded")
...
if "global_cookies" not in st.session_state:
    try:
        st.session_state.global_cookies = EncryptedCookieManager(
            password="movie_recommendation_secret_key_2024", prefix="firebase_"
        )
...
def main():
    ...
    # 쿠키 상태 확인
    cookies_ready = st.session_state.global_cookies is not None and st.session_state.global_cookies.ready()
    ...
    if recommendation_type == "⭐ 내 평점 관리":
        ...
        if firebase_available and cookies_ready:
            render_user_based_recommendation(
                recommender, df_movies, df_ratings_filtered, st.session_state.global_cookies
            )
