...
def enrich_search_results(search_results: list, df_movies: pd.DataFrame, include_cast: bool = True) -> list:
    """
...
def poster_search_by_text(
    ...
    try:
        ...
        if min_rating > 0 or min_vote_count > 0 or genre or language:
            ...
            if genre:
                ...
                filtered_df = filtered_df[
                    filtered_df[genre_col].apply(lambda x: any(g in str(x) for g in genre) if pd.notna(x) else False)
                ]
            ...
            if not filter_movie_ids:
                ...
                return PosterSearchResponse(
                    query_type="text", query=query, total_results=0, results=[], session_id=None
                )
        ...
        logger.info(
            f"🔍 포스터 텍스트 검색: '{query}' (limit={limit}, filter_ids={len(filter_movie_ids) if filter_movie_ids else 'All'})"
        )
        search_results = clip_service.search_by_text(text=query, k=limit, filter_movie_ids=filter_movie_ids)

        # 5. 검색 결과에 메타데이터 추가
        enriched_results = enrich_search_results(
            search_results=search_results, df_movies=df_movies, include_cast=include_cast
        )
