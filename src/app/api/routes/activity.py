...
async def log_click_event(request: Request, click_event: ClickEventRequest = Body(...)):
    ...
    try:
        ...
        logger.info(
            f"✅ 클릭 로깅 완료: session={click_event.session_id}, movie={click_event.movie_id}, pos={click_event.position}, type={click_event.link_type}"
        )
...
async def get_ctr_data(
    ip_filter: Optional[str] = None, start_date: Optional[str] = None, end_date: Optional[str] = None
):
    """
    CTR 분석 데이터 조회
    ...
    try:
        ...
        # CTR 데이터 조회
        ctr_data = activity_logger.get_ctr_data(ip_filter=ip_filter, start_date=start_dt, end_date=end_dt)
        ...
        raise HTTPException(status_code=500, detail=f"CTR 데이터 조회 중 오류가 발생했습니다: {str(e)}")
...
async def get_activity_stats(ip: Optional[str] = None):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"활동 통계 조회 중 오류가 발생했습니다: {str(e)}")
