...
### [🧪 실험] A/B 테스트 LLM 시뮬레이션

**배경**
- 실제 사용자 데이터 없이 추천 시스템의 A/B 테스트를 빠르게 평가할 방법 필요
- LLM을 활용하여 다양한 사용자 컨텍스트에서 추천 리스트의 선호도를 시뮬레이션

**작업 내용**
- `LLMEvaluator`를 사용하여 두 가지 추천 리스트(System A, System B) 비교
- 두 가지 사용자 컨텍스트로 실험:
  1. Sci-Fi 애호가 (mind-bending plots, visual effects 선호)
  2. 스릴러/다크 테마 선호자 (심리적 깊이, 긴장감 선호)

**결과**
- 두 사용자 컨텍스트 모두에서 **System B가 선호**됨
- Sci-Fi 애호가: System B의 최신 영화와 시각적 효과가 뛰어난 작품들이 선호
- 스릴러 선호자: System B의 심리적 깊이와 다크한 분위기 작품들이 더 잘 맞음
- System B는 더 다양한 장르 조합(sci-fi + thriller)과 최신 작품 포함으로 다양한 선호도에 대응

**배운 점**
- LLM 기반 시뮬레이션으로 실제 사용자 테스트 전에 빠른 가설 검증 가능
- 사용자 컨텍스트에 따라 선호도가 달라지므로 다양한 사용자 그룹에 대한 테스트 필요
- 장르 다양성과 최신성의 조합이 추천 품질에 영향

**참고**: 상세 실험 내용은 `docs/experiments/25-11-25-ab-test-simulation.md` 참고



## 2025-11-23 (토)

### [🔧 기능 구현] 사용자 활동 로깅 시스템 구축

**배경**
- CTR(Click-Through Rate) 예측 모델 학습을 위한 데이터 수집 필요
- 검색 후 어떤 영화를 클릭했는지 추적하여 사용자 행동 패턴 분석

**작업 내용**

1. **활동 로깅 인프라 구축**
   - `app/api/user_activity_logger.py` - 핵심 로깅 모듈 생성
   - IP 기반 사용자 식별 (로그인 불필요)
   - JSONL 형식으로 `app/activity_logs/`에 저장
   - 활동 타입: 검색, 클릭, 평점, 조회 등
...
2. **검색 로깅 구현**
   ...
   - 각 검색마다 고유 세션 ID 생성
   - 검색어, 결과 수, 결과 영화 ID 리스트 저장

3. **클릭 트래킹 구현**
   - IMDb 링크 클릭 시 자동 로깅
   - 검색 세션 ID와 클릭 연결
   - 클릭한 영화 ID, 검색 결과 내 순위 저장
   - JavaScript 이벤트 리스너로 프론트엔드 통합

4. **분석 API 엔드포인트**
   - `POST /activity/click` - 클릭 이벤트 기록
   - `GET /activity/ctr-data` - CTR 분석용 검색-클릭 데이터 조회
   - `GET /activity/stats` - 활동 통계 (검색 수, 클릭 수, CTR 등)




**다음 단계**
- [ ] 수집된 CTR 데이터로 머신러닝 모델 학습
- [ ] 평점, 영화 조회 등 다른 활동 로깅 추가 (선택적)

---


### [🔍 실험/테스트] 유저 클러스터링 결과 활용 방법 실험

- **고민하는 내용**
  - **클러스터링 결과를 어떻게 활용할까?**
  - **Cold Start 문제 해결**
    - 신규 유저가 들어왔을 때, 몇 가지 질문(선호 장르 등)을 통해 가장 가까운 클러스터에 할당
    - 해당 클러스터의 인기 영화나 선호 장르를 기반으로 초기 추천 제공
  - **Feature Engineering**
    - `cluster_id` 자체를 추천 모델(예: Deep Learning 모델)의 입력 피처로 사용
    - 유저의 성향을 나타내는 고수준의 정보로 활용 가능
  - **Hierarchical Models**
    - 각 클러스터별로 별도의 추천 모델을 학습
    - 예를 들어, '공포 영화 매니아' 클러스터에는 공포 영화에 특화된 모델 적용
  - **Diversity 확보**
    - 추천 결과가 특정 장르에 편중되지 않도록, 다양한 클러스터에서 선호하는 영화를 섞어서 추천
    - "이런 스타일의 영화는 어떠세요?" 와 같이 다른 클러스터의 영화를 제안하여 세렌디피티 증대
